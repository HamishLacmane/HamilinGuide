...
- **Expanded/modal card** (not a separate page, deliberately light on GitHub Pages): photo placeholder, name, location + cuisine, an open/closed status line, then **which guide category the restaurant belongs to** as a solid dark pill (e.g. "Gone But Not Forgotten") placed *above* the dish list and *below* the status line — grouped with the other structural metadata, not down with the personal tags. Then the **full per-dish tier breakdown**, one row per dish in "Tier — Dish name" format (e.g. "Gold — Spicy Chicken Fillet Burger"), each row background-tinted to match its tier. Then Hamish's own comment (placeholder), a couple of tags (placeholder), and the Google Maps link.
